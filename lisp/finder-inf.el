...
(setq package--builtins '(
  ...
  (org . [(7 8 3) nil "Outline-based notes management and organizer"])
...
(setq finder-keywords-hash
      #s(hash-table size 487 test eq rehash-size 1.5 rehash-threshold 0.8 data (abbrev (skeleton pcomplete emacs icomplete hippie-exp expand edmacro dabbrev completion autoarg) convenience (picture emacs makeinfo flyspell calc keypad cua-base diff-mode compare-w erc winner windmove vcursor tmm ruler-mode repeat minibuf-eldef mb-depth longlines linum kmacro json iswitchb imenu ido ibuffer hippie-exp follow filesets filecache ffap ebuff-menu desktop delsel dabbrev completion chistory calculator bs autorevert autoinsert align) languages (smie emacs xscheme vhdl-mode verilog-mode vera-mode tcl sql simula sh-script scheme ruby-mode ps-mode perl-mode pascal octave-mod modula2 mixal-mode meta-mode m4-mode ld-script js idlwave icon fortran flymake f90 executable delphi dcl-mode cwarn cperl-mode cfengine cc-mode cc-guess cap-words autoconf asm-mode antlr-mode ada-mode sgml-mode erc tempo skeleton paren ledit info-look allout align) lisp (unsafep trace testcover tabulated-list smie shadow re-builder pp map-ynp macroexp emacs ewoc ert elp elint eieio edebug checkdoc check-declare benchmark xscheme scheme inf-lisp hideshow inversion cedet strokes isearchb ielm color cmuscheme align) outlines (hideshow hideif org reveal outline foldout elide-head allout allout-widgets) wp (ebnf2ps underline two-column tildify emacs sgml-mode refill picture page-ext nroff-mode ispell enriched org handwrite whitespace ps-print printing longlines allout) pgp (gnus epg epa emacs allout) gnupg (gnus epg epa emacs allout) comm (bindat sql sgml-mode dns-mode url erc zeroconf webjump tramp tls telnet socks soap-client secrets rlogin rcirc net-utils ldap gnutls eudc dns dig dbus ange-ftp terminal talk shadowfile notifications master kermit ansi-color) processes (eshell bindat sql inf-lisp idlwave url erc tramp rcompile term shell server proced pcomplete emacs emacs-lock dirtrack cmuscheme ansi-color) terminals (tramp xt-mouse vt-control terminal ansi-color) services (ansi-color) help (emacs woman novice man makesum informat info info-look icomplete help-at-pt finder ehelp) files (po url compare-w wdired view emacs shadowfile recentf locate ffap doc-view arc-mode) archives (arc-mode) msdog (arc-mode) editing (hideshow dcl-mode arc-mode) major-mode (dcl-mode arc-mode) extensions (tq tabulated-list rx ring regi regexp-opt map-ynp ewoc eldoc emacs cust-print cl benchmark avl-tree assoc calc gnus cookie1 type-break tree-widget thingatpt tempo strokes skeleton ido emacs-lock electric ehelp array) emulations (emacs ws-mode viper vip vi tpu-edt edt cua-base crisp hl-line delsel autoarg) mouse (artist goto-addr browse-url xt-mouse emacs thingatpt t-mouse strokes msb mouse-sel mouse-drag mouse-copy ffap avoid) hardware (zeroconf dbus vt100-led emacs flow-ctrl battery) internal (warnings syntax smie emacs disass userlock select novice loadhist gs delim-col) bookmarks (saveplace bookmark) placeholders (saveplace bookmark) annotations (bookmark) tools (trace re-builder package ert elp eieio edebug copyright emacs check-declare which-func make-mode hideshow gud glasses gdb-mi flymake etags ebrowse bug-reference asm-mode vc smerge-mode log-view emerge ediff diff cvs-status add-log reporter mail-utils semantic xesam rcompile time-stamp tempo speedbar skeleton sb-image imenu format-spec find-file ezimage elide-head dframe calculator) i18n (emacs po gnus double descr-text) data (bindat remember page-ext url gnus org snmp-mode timeclock xml whitespace jka-compr emacs hexl hex-util cdl) faces (m4-mode ld-script cwarn enriched gnus erc emacs paren hl-line hilit-chg hi-lock face-remap descr-text color) color (color) hex (color) rgb (htmlfontify color) hsv (color) hsl (color) cie-lab (color) background (color) mule (emacs ps-print) multilingual (emacs descr-text) character\ composition (emacs) expand (dabbrev) completion (crm iswitchb dabbrev) unicode (emacs descr-text) file (dns-mode speedbar sb-image ezimage dframe) tags (ebrowse semantic speedbar sb-image ezimage dframe) dired (wdired emacs) window (emacs windmove follow) drag (emacs) drop (emacs) pdf (doc-view) ps (doc-view) dvi (doc-view) font (ps-print emacs) system-font (emacs) tool-bar-style (emacs) unix (sh-script make-mode gud gdb-mi executable cc-mode ispell emerge ediff diff telnet rlogin woman tar-mode sort proced emacs lpr locate find-lisp find-dired) mail (gnus mh-e emacs fortune unrmail undigest uce supercite smtpmail sendmail rmail rfc822 rfc2368 reporter mspools metamail mailpost mail-utils mailclient mailalias mailabbrev hashcash footnote feedmail mairix imap goto-addr time-date epa) message (gnus feedmail epa) face\ remapping (face-remap) display (hi-lock follow face-remap) user\ commands (face-remap) hypermedia (sgml-mode css-mode url org xesam soap-client quickurl browse-url emacs htmlfontify ffap) matching (regi re-builder thingatpt soundex misearch emacs hi-lock find-file ffap) filesets (filesets) c (hideshow hideif flymake cwarn cmacexp cc-mode cc-guess emacs find-file) folding (foldout) minor-mode (hi-lock follow) frames (winner emacs talk hl-line) generic (emacs) comment (emacs) font-lock (emacs) colour (htmlfontify) html (gnus htmlfontify) markup (htmlfontify) etags (htmlfontify) buffer (msb ibuffer) multimedia (gnus metamail thumbs mpc emacs image-file image-dired iimage) docs (lisp-mnt checkdoc texnfo-upd texinfo texinfmt makeinfo info-xref) keyboard (keypad cua-base kmacro) md4 (md4) utilities (midnight) menu (erc msb) uncomment (emacs) desktop (notifications) notifications (notifications) password (secrets password-cache) cache (password-cache) passphrase (secrets password-cache) key (password-cache) print (handwrite ps-print printing) postscript (ps-mode ebnf2ps gnus handwrite ps-print printing) bdf (ps-print) multibyte (ps-print) vi (repeat) repeat (repeat) minibuffer (crm savehist emacs) scroll (scroll-all) crisp (crisp scroll-all) brief (crisp scroll-all) lock (scroll-all) spreadsheet (testcover ses) dijkstra (ses) gpm (t-mouse) linux (t-mouse) news (gnus uudecode supercite metamail mail-utils footnote binhex newsticker netrc time-date timezone) timers (type-break) virtual\ cursor (vcursor) environment (wdired) renaming (wdired) movement (windmove) xml (nxml xml) calendar (gnus org todo-mode timeclock calendar icalendar diary-lib) holidays (calendar) util (time-date parse-time) todo (remember todo-mode) hypertext (browse-url) dns (dns-mode dig) bind (dig) dig (gnus dig) network (gnus network-stream net-utils dns) tls (gnus tls gnutls) ssl (gnus tls gnutls) encryption (gnutls) mh-e (goto-addr) www (webjump goto-addr) hmac (gnus hmac-md5 hmac-def) rfc2104 (gnus hmac-md5 hmac-def) hmac-md5 (hmac-md5) md5 (hmac-md5) keyed-md5 (hmac-md5) cram-md5 (sasl hmac-md5) searching (gnus mairix) rss (gnus newsticker) atom (newsticker) ntlm (sasl ntlm) sasl (sasl ntlm) digest-md5 (sasl) soap (soap-client) web-services (soap-client) firewalls (socks) gnutls (gnus tls) irc (org erc) chat (erc) client (erc) internet (erc) button (erc) url (url vc erc) regexp (nxml erc) logging (erc) timestamp (erc) oo (eieio inversion cedet) project (semantic ede) make (semantic ede) syntax (emacs semantic mode-local) codegeneration (srecode) scheme (org ede) vc (url vc smerge-mode pcvs log-view log-edit emerge ediff diff diff-mode cvs-status compare-w add-log ede) binhex (gnus binhex) maint (gulp edebug copyright emacs checkdoc check-declare texnfo-upd texinfo texinfmt reporter) email (rmail feedmail) queue (feedmail) sendmail (feedmail) spray (feedmail) smtp (feedmail) draft (feedmail) hashcash (gnus hashcash) history (mail-utils) mime (gnus metamail) spam (gnus mh-e rmail) filter (rmail) rmail (rmail) uce (uce) unsolicited\ commercial\ email (uce) uudecode (gnus uudecode) games (zone yow tetris studly spook solitaire snake pong mpuz morse meese life landmark hanoi gomoku gametree gamegrid fortune dunnet doctor dissociate decipher cookie1 bubbles bruce blackbox animate 5x5) puzzles (5x5) utils (fortune) cursive\ writing (handwrite) gomoku (landmark) neural\ network (landmark) adaptive\ search (landmark) chemotaxis (landmark) pcl-cvs (pcvs log-edit cvs-status) cvs (smerge-mode pcvs log-view log-edit cvs-status) status (cvs-status) tree (avl-tree cvs-status) patch (diff-mode) diff (diff-mode) comparing (ediff) merging (ediff) patching (ediff) commit (log-edit) log (log-view log-edit) rcs (log-view) sccs (log-view) release\ management (pcvs) revision\ control (smerge-mode) merge (smerge-mode) diff3 (smerge-mode) conflict (smerge-mode) ccl (emacs) character\ set (emacs) coding-system (emacs) multibyte\ character (emacs) category (emacs) fontset (emacs) isearch (emacs) tex (meta-mode tildify texnfo-upd texinfo texinfmt tex-mode reftex refbib bibtex bibtex-style org emacs) iso (emacs) latin (emacs) japanese (emacs) kinsoku (emacs) coding\ system (emacs) latex (bibtex emacs) charset (emacs) diagnosis (emacs) input\ method (emacs) quail (emacs) tit (emacs) cxterm (emacs) normalization (emacs) literate\ programming (org) reproducible\ research (org) r (org) statistics (org) comint (org) fortran (fortran f90 org) js (org) accounting (org) babel\ language (org) maxima (org) interactive\ shell (org) shen (org) org (org) task (org) remember (org) org-mode (org) docbook (org) erc (org) link (org) tables (org) plotting (org) emacsclient (org) taskjuggler (org) project\ planning (org) cancel-lock (gnus) sha1 (gnus) web (gnus) xpm (gnus) annotation (gnus) glyph (gnus) registry (gnus) synchronization (gnus) nntp (gnus) nnrss (gnus) external-body (gnus) partial (gnus) shar (gnus) forward (gnus) gnatsweb (gnus) gnus (gnus) s/mime (gnus) mml (gnus) ir (gnus) hz (gnus) hz+ (gnus) smime (gnus) x\.509 (gnus) pem (gnus) openssl (gnus) gmane (gnus) report (gnus) bogofilter (gnus) bbdb (gnus) dspam (gnus) whitelist (gnus) blacklist (gnus) spamassassin (gnus) bsfilter (gnus) ifile (gnus) stat (gnus) crm114 (gnus) spamoracle (gnus) yenc (gnus) cua (cua-base) mark (cua-base) bib (refer refbib bib-mode) bibtex (bibtex) conf (conf-mode) ini (conf-mode) windows (conf-mode) java (hideshow conf-mode) master (dns-mode) zone (dns-mode) soa (dns-mode) memory (remember) pim (remember) text (tildify) sgml (tildify) ada (ada-mode) project\ file (ada-mode) xref (ada-mode) antlr (antlr-mode) code\ generator (antlr-mode) awk (cc-mode) cc-mode (cc-mode) oop (cc-guess) perl (cperl-mode) dcl (dcl-mode) ebnf (ebnf2ps) c++ (hideshow ebrowse) f90 (f90) comments (hideshow) blocks (hideshow) hiding (hideshow) javascript (js) g++ (mantemp) templates (mantemp) metafont (meta-mode) metapost (meta-mode) knuth (mixal-mode) mix (mixal-mode) mixal (mixal-mode) asm (mixal-mode) mixvm (mixal-mode) the\ art\ of\ computer\ programming (mixal-mode) prolog (prolog) major (prolog) mode (prolog) sicstus (prolog) swi (prolog) mercury (prolog) ruby (ruby-mode) tcl (tcl) modes (tcl) vera (vera-mode) vhdl (vhdl-mode) mode-line (which-func) imenu (which-func) relaxng (nxml) burma (emacs) cham (emacs) chinese (emacs) cyrillic (emacs) czech (emacs) ethiopic (emacs) european (emacs) greek (emacs) korean (emacs) hanja (emacs) hebrew (emacs) indian (emacs) devanagari (emacs) khmer (emacs) lao (emacs) romanian (emacs) sinhala (emacs) slovak (emacs) tai\ viet (emacs) thai (emacs) word\ break (emacs) emacs (emacs) tibetan (emacs) vietnamese (emacs) data\ structures (avl-tree) avl (avl-tree) multiple\ elements (crm) debugging (elp) emacs-lisp (find-func) functions (find-func) variables (find-func) compiler (macroexp) macros (macroexp) strings (rx regexp-opt) regexps (rx regexp-opt) parsing (smie) indentation (smie) utility (unsafep testcover) safety (unsafep testcover))))

(provide 'finder-inf)
;; Local Variables:
